...
type TouchEvent<S extends WechatMiniprogram.IAnyObject, D extends WechatMiniprogram.IAnyObject = {}, M extends WechatMiniprogram.IAnyObject = {}> = WechatMiniprogram.TouchEvent<D, M, S>;
type SwiperTransitionEvent<D extends WechatMiniprogram.IAnyObject = {}, M extends WechatMiniprogram.IAnyObject = {}> = WechatMiniprogram.SwiperTransition<M, D>;
type SwiperAnimationFinishEvent<D extends WechatMiniprogram.IAnyObject = {}, M extends WechatMiniprogram.IAnyObject = {}> = WechatMiniprogram.SwiperAnimationFinish<M, D>;
...
interface CalendarEventHandlers {
     ...
     */
    swiperTrans(event: SwiperTransitionEvent<{
        type: CalendarSwiperType;
    }>): void;
    /**
     * [WebView] 处理周/月面板的swiper滑块变动结束
     */
    swiperTransEnd(event: SwiperAnimationFinishEvent<{
        type: CalendarSwiperType;
    }>): void;
    /**
...
export interface CalendarMethod extends WechatMiniprogram.Component.MethodOption, CalendarInitialize, CalendarEventHandlers, CalendarTrigger {
    /**
     * 刷新周/月面板数据
     * 单独写这个方法是worklet的需要
     */
...
export type CalendarInstance = WechatMiniprogram.Component.Instance<CalendarData, CalendarProp, CalendarMethod, CalendarCustomProp>;
...
export interface CalendarExport extends WechatMiniprogram.IAnyObject {
     ...
     * 获取插件
     */
    getPlugin<T extends Array<PluginConstructor>, K extends DefaultPluginKeyExtend<T> = DefaultPluginKeyExtend<T>>(key: K): Voidable<DefaultPluginMapExtend<T>[K]>;
    /**
     * 更新插件日期数据
     */
 ...
 * 组件工具类超类
 */
export declare class CalendarHandler {
    protected _instance_: CalendarInstance;
    constructor(component: CalendarInstance);
    protected get _render_(): "skyline" | "webview" | undefined;
}
export {};
